...
test('#componentDidMount creates foundation', () => {
  const wrapper = shallow(<Icon><i /></Icon>);
...
test('#componentDidUpdate doesn\'t call #foundation.setDisabled if ' +
  ...
  td.verify(wrapper.instance().foundation_.setDisabled(td.matchers.isA(Boolean)), {times: 0});
});
...
test('#adapter.getAttr for role', () => {
  const wrapper = shallow(<Icon><i role='button'/></Icon>);
...
test('#adapter.getAttr for tabIndex works with Custom Component', () => {
  const wrapper = shallow(<Icon><MaterialIcon icon='favorite' tabIndex='0'/></Icon>);
  const tabIndex = wrapper.instance().foundation_.adapter_.getAttr('tabindex');
  assert.equal(tabIndex, '0');
});

test('#adapter.getAttr for role works with Custom Component', () => {
  const wrapper = shallow(<Icon><MaterialIcon icon='favorite' role='button'/></Icon>);
  const role = wrapper.instance().foundation_.adapter_.getAttr('role');
  assert.equal(role, 'button');
});
