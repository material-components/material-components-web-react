...
test('classNames adds classes', () => {
  const wrapper = shallow(
    <LineRipple className='test-class-name' />);
...
test('#adapter.hasClass returns true if exists in classList', () => {
  ...
  const hasClass = wrapper.instance().foundation_.adapter_.hasClass('test-color-class');
...
test('onTransitionEnd calls the #foundation.handleTransitionEnd', () => {
  ...
  td.verify(
    wrapper.instance().foundation_.handleTransitionEnd(event), {times: 1});
});
