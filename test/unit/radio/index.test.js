...
test('renders label if props.label is provided', () => {
  const wrapper = shallow(<Radio label='meow'><NativeRadioControl /></Radio>);
...
test('calls foundation.setDisabled if child.props.disabled is true', () => {
  ...
  const wrapper = mount(<Radio><NativeRadioControl disabled/></Radio>);
...
test('sets state.nativeControlId if child has props.id', () => {
  const wrapper = shallow(<Radio className='test-class-name'><NativeRadioControl id='123' /></Radio>);
  assert.equal(wrapper.state().nativeControlId, '123');
});

test('calls props.initRipple', () => {
  const initRipple = td.func();
  const wrapper = mount(<Radio initRipple={initRipple}><NativeRadioControl /></Radio>);
  const input = wrapper.childAt(0).childAt(0).childAt(0).getDOMNode();
  const radio = wrapper.childAt(0).childAt(0).getDOMNode();
  td.verify(initRipple(radio, input), {times: 1});
});

test('renders label with for attribute tied to native control id', () => {
  const wrapper = shallow(<Radio label='meow'><NativeRadioControl id='123' /></Radio>);
...
test('calls foundation.setDisabled if children.props.disabled updates', () => {
  ...
  wrapper.children().instance().foundation_.setDisabled = td.func();
  wrapper.setProps({disabled: true});
  td.verify(wrapper.children().instance().foundation_.setDisabled(true), {times: 1});
});
...
test('calls foundation.setDisabled if children.props.disabled updates to false', () => {
  ...
  td.verify(wrapper.children().instance().foundation_.setDisabled(false), {times: 1});
});
