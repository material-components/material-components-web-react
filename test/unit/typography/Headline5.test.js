...
test('classNames adds classes', () => {
  const wrapper = shallow(<Headline5 className='test-class-name'>Text</Headline5>, {disableLifecycleMethods: true});
...
test('renders a different tag', () => {
  const wrapper = shallow(<Headline5 tag="p">Text</Headline5>, {disableLifecycleMethods: true});
  assert.equal(wrapper.type(), 'p');
});

test('keeps custom props', () => {
  const wrapper = shallow(
    <Headline5 propOne={true} propTwo='test-prop'>Children</Headline5>,
