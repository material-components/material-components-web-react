...
test('classNames adds classes', () => {
  const wrapper = shallow(<Headline6 className='test-class-name'>Text</Headline6>, {disableLifecycleMethods: true});
...
test('renders a different tag', () => {
  const wrapper = shallow(<Headline6 tag="p">Text</Headline6>, {disableLifecycleMethods: true});
  assert.equal(wrapper.type(), 'p');
});

test('keeps custom props', () => {
  const wrapper = shallow(
    <Headline6 propOne={true} propTwo='test-prop'>Children</Headline6>,
