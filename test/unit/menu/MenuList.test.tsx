...
test('#handleSelect calls props.handleSelect', () => {
  const handleSelect = td.func<(activatedItemIndex: number, selected: MDCListIndex) => void>();
...
test('#handleSelect calls props.handleItemAction', () => {
  ...
  const wrapper = mount(<MenuList handleItemAction={handleItemAction}>
    <button className='mdc-list-item'>text</button>
  </MenuList>);
  coerceForTesting<MenuList>(wrapper.instance()).handleSelect!(0, 0);
  td.verify(handleItemAction(wrapper.find('.mdc-list-item').getDOMNode()), {times: 1});
});
